...
import { FIELD_LOGO_FOLDER_PATH } from "../upload-folder-path";

@Injectable()
export class UploaderFieldLogoService extends IUploaderImage {
    ...
    run(imageFile: FormData, cropped: Cropped): void {
        ...
        this.upload.run(imageFile, cropped, FIELD_LOGO_FOLDER_PATH.test).subscribe({
            next: (response) => {
